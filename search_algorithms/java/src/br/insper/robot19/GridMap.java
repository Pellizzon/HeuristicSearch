...
public class GridMap {
	...
	public void setRoute(int row, int col) {
		...
		if (row >= 0 && row < path.length && col >= 0 && col <= path[0].length && path[row][col] != BlockStatus.START
				&& path[row][col] != BlockStatus.GOAL) {
	...
	/**
	 * Determines the position of the block given an action
	 * 
	 * @param current
	 * @param action
	 * @return the next block, or null if the robot leaves the map area
	 */
	public Block nextBlock(Block current, RobotAction action) {
		int row = current.row;
		int col = current.col;
	...
	public void drawWall(float x0, float y0, float x1, float y1) {
		...
		// Converts coordinates to grid cells
	...
	private void bresenhamX(int x0, int y0, int x1, int y1) {
		...
		int x = x0;
		int y = y0;
		for (; x >= 0 && y >= 0 && x < grid[0].length && y < grid.length && x != x1; x += Math.signum(deltax)) {
	...
	private void bresenhamY(int x0, int y0, int x1, int y1) {
		...
		int x = x0;
		int y = y0;
		for (; x >= 0 && y >= 0 && x < grid[0].length && y < grid.length && y != y1; y += Math.signum(deltay)) {
			setType(y, x, BlockType.WALL);
			error += deltaerr;
			if (error >= 0.5) {
